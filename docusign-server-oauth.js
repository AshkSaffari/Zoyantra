BUTTON BACK WITH SINERS NAME AND ........const express = require('express');
...
app.get('/docusign/auth', (req, res) => {
  // Use the correct redirect URI for the Zoyantra app (port 3000)
  const redirectUri = 'http://localhost:3000/docusign/callback';
  const authUrl = `${DOCUSIGN_CONFIG.authServer}/oauth/auth?response_type=code&scope=signature&client_id=${DOCUSIGN_CONFIG.clientId}&redirect_uri=${redirectUri}`;
...
async function handleDocuSignCallback(code, res) {
  ...
  // Return JSON for POST requests, HTML for GET requests
  if (req.method === 'POST') {
...
app.get('/api/docusign/status', (req, res) => {
  res.json({
    ...
    user: docuSignToken ? { name: 'DocuSign User (OAuth)' } : null,
    authType: 'OAuth2'
  });
});
...
app.post('/api/docusign/send-for-signature', async (req, res) => {
  try {
    if (!docuSignToken) {
      return res.status(401).json({ error: 'DocuSign not connected. Please authenticate first.' });
    }
    ...
    apiClient.addDefaultHeader('Authorization', `Bearer ${docuSignToken.access_token}`);
    ...
    res.status(500).json({ error: 'Failed to send document for signature: ' + error.message });
  }
});
...
app.listen(PORT, () => {
  console.log(`🚀 DocuSign OAuth Integration Server running on port ${PORT}`);
  ...
  console.log(`🔐 DocuSign Authentication: OAuth2`);
  console.log(`🔗 To authenticate DocuSign, visit: http://localhost:${PORT}/docusign/auth`);
});

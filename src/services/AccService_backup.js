class AccService {
  ...
  async detectRegion(hubId) {
    try {
      ...
      // ACC hub and project handling - use dynamic hub detection instead of hardcoded IDs
      const knownHubs = [
        // Removed hardcoded hub IDs to prevent 404 errors
        // Hub IDs will be detected dynamically from API responses
      ];
  ...
  async getHubs() {
    try {
      console.log('🔍 Fetching hubs with current token...');
      console.log('🔍 Token type:', this.accessToken ? 'Present' : 'Missing');
      ...
      // Also try the admin API to get more comprehensive hub list
      try {
        console.log('🔍 Trying admin API for additional hubs...');
        const adminResponse = await this.makeRequest('/construction/admin/v1/hubs');
        console.log('🔍 Admin Hubs API Response:', adminResponse);
      } catch (adminError) {
        console.log('⚠️ Admin API not accessible:', adminError.message);
      }
  ...
  async getAllAccessibleHubs() {
    try {
      ...
      console.log('🎯 Total unique hubs found:', finalHubs.length);
      console.log('📋 Hub names:', finalHubs.map(h => `${h.name} (${h.source})`));
  ...
  async getProjects(hubId) {
    try {
      ...
      // Special handling for APAC/Australian hubs
      const isAPACHub = hubId.includes('ddc5f6e2-9ddc-475e-82ed-b05da4ac18c2') || 
                       hubId.includes('0ad34203-f5d6-481b-8829-97536f094965') ||
                       hubId.toLowerCase().includes('apac') ||
                       hubId.toLowerCase().includes('australia');
      
      if (isAPACHub) {
        console.log('🌏 APAC/Australian hub detected - using alternative API approach');
        ...
        // If all regions failed, try alternative APIs for APAC hubs
        console.log('🔄 All regions failed, trying alternative APIs...');
        const alternativeAPIs = [
          {
            name: 'Data Management API',
            url: `/data/v1/projects`,
            method: 'GET'
          },
          {
            name: 'BIM 360 Docs API',
            url: `/bim360/docs/v1/projects`,
            method: 'GET',
            headers: {
              'x-user-id': this.credentials.accountId || 'default'
            }
          },
          {
            name: 'Construction Admin API',
            url: `/construction/admin/v1/projects`,
            method: 'GET'
          }
        ];
        ...
        for (const api of alternativeAPIs) {
          try {
            console.log(`🔄 Trying ${api.name}...`);
            const response = await this.makeRequest(api.url, api.method, null, api.headers || {});
            console.log(`✅ Success with ${api.name}:`, response);
        ...
        // If all approaches fail, throw a helpful error
        throw new Error(`Unable to access APAC hub ${hubId}. This hub may require special permissions or different authentication. Please contact your Autodesk administrator.`);
      }
...
4. The project is in a different region`);
  ...
  async createExpenseFromTimesheet(projectId, timesheetData) {
    try {
      ...
      const expenseItemPayload = {
        name: timesheetData.taskName || 'zero',
        description: `Work performed: ${timesheetData.taskDescription || timesheetData.taskName || 'zero'}`,
        ...
        scope: 'partial',
        quantity: timesheetData.hours || 1,
        unitPrice: timesheetData.hourlyRate || timesheetData.rate || 0,
        unit: 'hr',
        amount: timesheetData.amount || (timesheetData.hours || 1) * (timesheetData.hourlyRate || timesheetData.rate || 0),
        budgetId: timesheetData.budgetId,
        budgetCode: timesheetData.budgetCode,
        externalId: `timesheet_item_${timesheetData.id || Date.now()}`,
        externalSystem: 'Zoyantra Timesheet System',
        externalMessage: `Timesheet: ${timesheetData.hours || 0} hours at $${timesheetData.hourlyRate || timesheetData.rate || 0}/hr = $${timesheetData.amount || (timesheetData.hours || 1) * (timesheetData.hourlyRate || timesheetData.rate || 0)}`,
        integrationState: null
      };
  ...
  async createExpenseWithContainer(containerId, expenseData) {
    try {
      ...
      const expensePayload = {
        ...
        scope: expenseData.scope || 'partial', // full or partial
  ...
  async updateExpense(projectId, expenseId, expenseData) {
    try {
      ...
      let containerId = projectId;
      
      if (projectId.startsWith('b.')) {
        // BIM 360 project - convert to container ID format
        const cleanProjectId = projectId.substring(2);
        containerId = `c.${cleanProjectId}`;
        console.log(`🔧 Converted BIM 360 project ${projectId} to container ID: ${containerId}`);
      } else if (projectId.startsWith('a.')) {
        // ACC project - use as is
        containerId = projectId;
        console.log(`🔧 Using ACC project as container ID: ${containerId}`);
      } else {
        // Raw UUID - convert to container ID format
        containerId = `c.${projectId}`;
        console.log(`🔧 Converted raw UUID ${projectId} to container ID: ${containerId}`);
      }
      
      console.log(`💰 Updating expense ${expenseId} for container: ${containerId}`);
      console.log('📝 Update payload:', expenseData);
      
      // Use the container ID for the API call
      const response = await this.makeRequest(`/cost/v1/containers/${containerId}/expenses/${expenseId}`, 'PATCH', expenseData);
      console.log(`✅ Expense updated successfully:`, response);
      return response;
    } catch (error) {
      console.error('❌ Error updating expense:', error);
  ...
  async getMainContracts(projectId, options = {}) {
    try {
      // For Cost Management API, we need to use the container ID (c. prefix)
      // Convert project ID to container ID
      const cleanProjectId = projectId.startsWith('b.') ? projectId.substring(2) : projectId;
      const containerId = `c.${cleanProjectId}`;
      console.log(`📋 Getting main contracts for project: ${cleanProjectId} (container: ${containerId})`);
      
      let url = `/cost/v1/containers/${containerId}/main-contracts`;
      const queryParams = [];
